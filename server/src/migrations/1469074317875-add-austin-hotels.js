...
let lasVegasId = "austin";
let imagePath = "assets/1469074317875-add-austin-hotels";

exports.up = function(next) {
	let listHotels = require(__dirname + '/assets/AustinHotels.json');
    listHotels.map(hotel => {
        hotel.dipLocation = lasVegasId;
...
exports.down = function(next) {
    connectionPromise.then(connection => {
        connection.db.collection('hotels', (error, collection) => {
            collection.find({dipLocation: lasVegasId}).toArray((error, hotels) => {
                if (error) {
                    return next(error);
                }
                var listServices = [];
                ...
                collection.remove({dipLocation: lasVegasId}, (error) => {
                    if (error) {
                        return next(error);
                    }
...
function saveListHotels(hotelImages, poolImages, hotels, next) {
    ...
    console.info('begin inserting Austin hotel');
