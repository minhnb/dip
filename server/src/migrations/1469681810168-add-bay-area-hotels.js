...
let locationId = "san_francisco_bay_area";
let imagePath = "assets/1469681810168-add-bay-area-hotels";

exports.up = function(next) {
	let listHotels = require(__dirname + '/assets/BayAreaHotels.json');
    listHotels.map(hotel => {
        hotel.dipLocation = locationId;
...
exports.down = function(next) {
    connectionPromise.then(connection => {
        connection.db.collection('hotels', (error, collection) => {
            collection.find({dipLocation: locationId}).toArray((error, hotels) => {
                if (error) {
                    return next(error);
                }
                var listServices = [];
                ...
                collection.remove({dipLocation: locationId}, (error) => {
                    if (error) {
                        return next(error);
                    }
...
function createPoolService(hotel, poolImage) {
    ...
    hotelService.amenities = hotel.amenities;
...
function saveListHotels(hotelImages, poolImages, hotels, next) {
    ...
    console.info('begin inserting Bay Area hotel');
