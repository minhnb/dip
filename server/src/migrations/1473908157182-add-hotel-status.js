...
const up = async (function(next) {
    ...
    await (updateDocuments(collection, {active: false, submission: {$exists: false}}, {
        $set: {submission: {
            status: 'Initial'
        }}
    }));
    await (updateDocuments(collection, {active: true, submission: {$exists: false}}, {
        $set: {submission: {
            status: 'Approved'
        }}
    }));
...
const down = async (function(next) {
    let collection = await (getCollection('hotels'));
    await (updateDocuments(collection, {}, {$unset: {submission: 1}}));
    return next();
});
