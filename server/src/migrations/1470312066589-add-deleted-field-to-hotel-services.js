...
const up = async (function(next) {
    try {
        let collection = await(getCollection('hotelservices'));
        let updatedHotelServices = await(updateDocuments(collection, {deleted: {$exists: false}}, {$set: {deleted: false}}));
        console.log('updated hotel services: ' + updatedHotelServices.result.nModified);
        return next();
    } catch (ex) {
        return next(ex);
    }
